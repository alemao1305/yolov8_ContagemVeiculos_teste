...
# Dicionário de níveis de confiança por classe de veículo
...
# Inicializar somas de confiança e contadores de detecções
confidence_sums = {"car": 0, "motorcycle": 0, "bus": 0, "truck": 0, "bicycle": 0}
detection_counts = {"car": 0, "motorcycle": 0, "bus": 0, "truck": 0, "bicycle": 0}
...
# Função para resetar a contagem de veículos e somas de confiança
def reset_vehicle_count():
    global vehicle_count, confidence_sums, detection_counts
    vehicle_count = {"car": 0, "motorcycle": 0, "bus": 0, "truck": 0, "bicycle": 0}
    confidence_sums = {"car": 0, "motorcycle": 0, "bus": 0, "truck": 0, "bicycle": 0}
    detection_counts = {"car": 0, "motorcycle": 0, "bus": 0, "truck": 0, "bicycle": 0}
        ...
        for detection in results[0].boxes:
            ...
            if class_id in class_names:
                ...
                if confidence >= confidence_thresholds[class_name]:
                    ...
                    cv2.putText(roi, f"{class_names[class_id]}: {confidence:.2f}", (bbox_x1, bbox_y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                    
                    class_name = class_names[class_id]
                    confidence_sums[class_name] += confidence.item()
...
while True:
    ...
    for vehicle, count in vehicle_count.items():
        avg_confidence = confidence_sums[vehicle] / detection_counts[vehicle] if detection_counts[vehicle] > 0 else 0
        text = f"{vehicle}: {count}" #(Confianca media: {avg_confidence:.2f})"
    ...
    if cv2.waitKey(1) & 0xFF == 27 or cv2.getWindowProperty('Video', cv2.WND_PROP_VISIBLE) < 1:
        break
