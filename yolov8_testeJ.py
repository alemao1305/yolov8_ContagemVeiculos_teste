...
# Função para atualizar a lista de veículos contados
def update_counted_vehicles():
    global counted_vehicles
    for vehicle_id in list(counted_vehicles.keys()):
        if counted_vehicles[vehicle_id]["center_y"] > roi_h:
...
while True:
    ...
    if results and results[0].boxes:
        ...
        for detection in results[0].boxes:
            ...
            if confidence >= confidence_threshold and class_id in class_names:  # Filtrar por classes de interesse (carro, moto, ônibus, caminhão)
                ...
                vehicle_id = f"{bbox_x1}-{bbox_y1}-{bbox_center_x}-{bbox_center_y}"
                if check_line_crossing(bbox_center_y, roi_h // 2, vehicle_id):
                    counted_vehicles[vehicle_id] = {"center_y": bbox_center_y, "class_id": class_id}
    ...
    # Atualizar a lista de veículos contados
    update_counted_vehicles()
